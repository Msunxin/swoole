...
class SOA
{
        ...
        if ($retObj->socket != null)
        {
            while (true) {
                ...
                if (!isset($data)) {
                    ...
                    $header = unpack(SOAServer::HEADER_STRUCT, substr($recv, 0, SOAServer::HEADER_SIZE));
                    //错误的包头
                    if ($header === false or $header['length'] <= 0) {
                    ...
                    $data = substr($recv, SOAServer::HEADER_SIZE);
                } else {
                    $data .= $recv;
                }
                //达到规定的长度
                if (strlen($data) == $header['length']) {
                    //成功处理
                    $this->finish(SOAServer::decode($data, $header['type']), $retObj);
                    break;
                }
                //继续等待数据
            }
    ...
    /**
     * 调用远程api
     * @param string $api 示例：/order/getOrderDetail
     * @param array $params api方法参数
     * @return mixed
     */
    public function callApi($api, $params = array())
    {
        return $this->sync($api, $params);
    }

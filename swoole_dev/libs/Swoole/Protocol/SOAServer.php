...
class SOAServer extends Base implements Swoole\IFace\Protocol
{
    ...
    private $cachedControllers = array();
    ...
    protected function call($request)
    {
        ...
        $callable = $request['call'];
        if ($request['call'][0] == '/')
        {
            $callable = $this->parseApi($request['call']);
        }
        ...
        parent::log("{$request['call']}: use " . number_format((microtime(true) - $startMicroTime) * 1000, 4) . "ms");
        if ($ret === false)
        {
